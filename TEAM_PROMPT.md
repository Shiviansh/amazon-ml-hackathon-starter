...
Expected default files are `data/raw/train.csv`, `data/raw/test.csv`, and `data/raw/sample_submission.csv`. The built-in profile uses `PRODUCT_ID`, target `PRICE`, text columns `TITLE`, `DESCRIPTION`, `PACK_SIZE`, and categorical columns `CATEGORY`, `BRAND`. For the exact 2025-style schema (`sample_id`, `catalog_content`, `image_link`, `price` in train; `sample_id`, `catalog_content`, `image_link` in test), inspect and use `src/prepare_challenge_data.py` to make canonical copies. It preserves the concatenated catalog text in `TITLE` and leaves separate description/brand/category/pack fields blank; it does not parse them. For any other format, inspect the real files and official task and adapt deliberately. Do not fabricate missing challenge data or assume these fields are guaranteed by this year's dataset.
